...
```text
The distance in point 1 and point 2 calculate the Euclidean distance between two points and the findcenter computes the center point of a given box. Mthod #3 returns a numeric value while method #4 returns a point which represents the center of a rectangle. 
```
...
```text
P1 and P2 are not aliased, P1 represents the center of the original box1 and the coordinates are (5, 8), but when box1 is modified the new coordinates become (5, 7) which is in p2. They represent different points in time and p1 and p2 are different points. 
```
